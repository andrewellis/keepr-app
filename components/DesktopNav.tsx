  ...
  return (
    <header className="hidden md:flex items-center justify-between px-8 py-4 border-b border-border bg-background sticky top-0 z-50">
      <Link href="/home" className="flex items-center gap-2">
        ...
        <span className="font-semibold text-base text-foreground">K33pr</span>
      </Link>
      <div className="flex items-center gap-2 px-3 py-2 border border-border rounded-lg w-[340px]">
        ...
        <input type="text" placeholder="Search for a product..." className="flex-1 bg-transparent text-sm text-foreground placeholder:text-muted-foreground outline-none" value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && query.trim()) { router.push('/scan?q=' + encodeURIComponent(query.trim())); setQuery('') } }} />
      </div>
      <div className="flex items-center gap-5">
        <Link href="/how-it-works" className="text-sm text-muted-foreground hover:text-foreground transition-colors">How it works</Link>
        <Link href="/faq" className="text-sm text-muted-foreground hover:text-foreground transition-colors">FAQ</Link>
        <Link href="/blog" className="text-sm text-muted-foreground hover:text-foreground transition-colors">Blog</Link>
        <Link href="/login" className="text-sm font-medium" style={{ color: '#534AB7' }}>Sign in</Link>
      </div>
    </header>
  )
}
